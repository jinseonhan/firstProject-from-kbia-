...
function selectTrendList() {
	...
	//등록일 미입력시
	if(startTrendDate == '' || startTrendDate == null){
		alert("시작날짜 입력해주세요");
		$("#startTrendDate").focus();
		return;
	}
	if(endTrendDate == '' || endTrendDate == null){
		alert("종료날짜 입력해주세요");
		$("#endTrendDate").focus();
		return;
	}
...
function tabGrid(inData) {
	//Build Tabulator
	    ...
	    columns:[
	        ...
	        {title:"제목", field:"title", width:600, hozAlign:"center"},
	        {title:"사용 여부", field:"useYn",hozAlign:"center", width:90},
