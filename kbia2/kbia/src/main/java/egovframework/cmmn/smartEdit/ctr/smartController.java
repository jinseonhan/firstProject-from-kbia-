 ...
 */
package egovframework.cmmn.smartEdit.ctr;
...
import egovframework.cmmn.smartEdit.vo.smartVO.Editor;
	...
	public String file_uploader(HttpServletRequest request, HttpServletResponse response, Editor editor){ 
		...
		try { 
			if(editor.getFiledata() != null && editor.getFiledata().getOriginalFilename() != null && !editor.getFiledata().getOriginalFilename().equals("")) {
				...
				String filename_ext = name.substring(name.lastIndexOf(".")+1); filename_ext = filename_ext.toLowerCase(); String[] allow_file = {"jpg","png","bmp","gif"}; 
				int cnt = 0; 
				...
				if(cnt == 0) { 
					...
					//파일 기본경로 _ 상세경로 
					String filePath = dftFilePath + "resouces"+ File.separator + "smarteditor2" + File.separator +"upload" + File.separator; 
					...
					return3 += "&sFileName="+ name; return3 += "&sFileURL=/resouces/smarteditor2/upload/"+realFileNm; 
				}
			}else { 
	...
	public void file_uploader_html5(HttpServletRequest request, HttpServletResponse response){ 
		try { 
			...
			if(cnt == 0) { 
				...
				String dftFilePath = request.getSession().getServletContext().getRealPath("/"); 
				//파일 기본경로 _ 상세경로 
				String filePath = dftFilePath + "resouces" + File.separator + "smarteditor2" + File.separator +"multiupload" + File.separator; 
				File file = new File(filePath); 
				if(!file.exists()) { 
					file.mkdirs(); 
				} 
				...
				sFileInfo += "&sFileURL="+"/resouces/smarteditor2/multiupload/"+realFileNm; 
				PrintWriter print = response.getWriter(); 
