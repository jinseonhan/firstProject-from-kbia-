...
function fileChange(data){
	for(var a = 0; a<data.length; a++){
		if(data[a].delYn=="N"){
			...
			$("#fileNameList").append("<div id='seachFile"+a+"'><label>"+ data[a].ognFileNm + "</label><a onclick='delIndustry("+a+");'> [삭제]</a><br></div>");
		}
	}
};
...
function update(){
	...
	if(industryTitle == null || industryTitle == ""){
		...
		$("#industryTitle").focus();
		return;
	}else if (industryContent == null || industryContent =="" || industryContent == '<div align="" style=""><p><br></p></div><p><br></p>'){
		alert("내용을 입력해주세요");
	...
	updateData.append("seqId","NOTICE_SEQ");
	updateData.append("refType", "INDUSTRY");
